...
class KanbanEnv(gym.Env):
    ...
    def set_contents(self):
        ...
        for colour in ['R','B','G','Y']:
            for value in range(1, 6):
                self.contents.append({'tile': Kanban, 'info': {'name': f'{colour}{value}kanban', 'colour': colour, 'value': value}, 'count': 2} )
            self.contents.append({'tile': Kanban, 'info': {'name': f'{colour}Xkanban', 'colour': colour, 'value': 0}, 'count': 1} )
    ...
    def rules_move(self):
        raise Exception('Rules based agent is not yet implemented for Kanban!')
